...
void ViconToGeo::SetUavs(int numUAV){
	
	int temp = this->numUAV;
	
	this->numUAV = numUAV; 

	//create new uav objects for each uav 
	for(int i = temp; i < numUAV; i++){
...
void ViconToGeo::CorrectMeasurements(float &x, float &y, float &q){
	
	float theta = MEASURED_ROOM_ROTATION;	//Rotating anti-clockwise around the origin in radians

	float cs = cos(theta);
	float sn = sin(theta);
	float xNew = x*cs - y*sn; 		//New x value
	float yNew = x*sn + y*cs;		//New y value
	float qNew = q + theta;			//New q value (might be + or -, can't think right now)
